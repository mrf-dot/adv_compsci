...
//			if the word meets neither of the above cases, return the same word
   ...
   {
      word = clean(word);
      if (englishToPirate.containsKey((word = word.toLowerCase())))
	      return englishToPirate.get(word);
      word = word.replaceAll("ing$", "in-").replace('v', '-');
      return word;
   }
...
//			another element might have the key "there" and the value ["thar"] 
   ...
   public Map<String, ArrayList<String>> flip()
   {
      ...
      for (String s: englishToPirate.values())
	      ans.put(s, new ArrayList<String>());
      for (String s: englishToPirate.keySet()) 
	      ans.get(englishToPirate.get(s)).add(s);
      return ans;
   }
